...
for i, ithnq in enumerate(thnq_arr):
    ...
    fout_name = './updated_%s_models/theoryXsec_%s%s_thnq%s_combined_TEST.data' %(model, theory, model, thnq_f )
    ...
    if model=="PWIA":
        ...
        for j in range(len(pm_avg1)):
            fout.write('%f   %f   %.12e   %.12e\n' % (thnq_bin[j], pm_avg2[j], red_pwiaXsec_avg[j], red_fsiXsec_avg[j]))
