...
def read_theoretical_models(theory="", model="", thnq=0):
    ...
    thnq_f = "%.2f" %(thnq)
    fname = './theoretical_models/updated_FSI_models/theoryXsec_%sFSI_thnq%s_combined_TEST.data' % (theory, thnq_f) #this file only reads CD-Bonn/AV18. JML will bw read separately.
    try:
        kin = dfile(fname)
    except:
