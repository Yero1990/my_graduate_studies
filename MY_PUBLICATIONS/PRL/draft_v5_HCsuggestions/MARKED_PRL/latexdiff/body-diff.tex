...
AV18 (green)\DIFdelbeginFL \DIFdelFL{and }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{, }\DIFaddendFL CD-Bonn (magenta) \DIFaddbeginFL \DIFaddFL{and WJC2 (orange) }\DIFaddendFL NN potentials.}
\label{fig:fig1}
\end{figure}
\end{center}
\twocolumngrid
...
cross sections using the charge-dependent Bonn (CD-Bonn) \cite{PhysRevC.63.024001}, Argonne $v_{18}$ (AV18) \cite{PhysRevC.51.38}\DIFdelbegin \DIFdel{and Paris \mbox{%DIFAUXCMD
...
\DIFaddbegin \DIFadd{For the WJC2 (orange) potential, the calculations were done by Orden and Jeschonnek \mbox{%DIFAUXCMD
\cite{PhysRevC.90.064006} }\hspace{0pt}%DIFAUXCMD
using a Bethe-Salpeter-like formalism for two-body bound states. The calculations
used two different form factor parametrizations (GKex05 \mbox{%DIFAUXCMD
\cite{PhysRevC.66.045501}}\hspace{0pt}%DIFAUXCMD
, AMT \mbox{%DIFAUXCMD
\cite{PhysRevC.76.035205}}\hspace{0pt}%DIFAUXCMD
) where a discrepancy of $\sim5.8-6.6 \%$ was found between the respective
WJC2 reduced cross sections. Figure \ref{fig:fig1} shows the results using only the WJC2 (GKex05) parametrization. }\DIFaddend \\
...
\indent Beyond $p_{\mathrm{r}}\sim250$ MeV/c at $\theta_{nq}=35^{\circ}$ and $45^{\circ}$ (Figs. \hyperref[fig:fig1]{1(a), 1(b)}), the Paris\DIFdelbegin \DIFdel{and
}\DIFdelend \DIFaddbegin \DIFadd{,
}\DIFaddend AV18 \DIFaddbegin \DIFadd{and WJC2 }\DIFaddend models increasingly differ from the CD-Bonn calculation. In this region, the Paris/AV18 cross sections are dominated by the PWIA and in good agreement up to $p_{\mathrm{r}}\sim700$ MeV/c \DIFaddbegin \DIFadd{whereas
the WJC2 PWIA falls off with a comparatively smaller cross section at $\theta_{nq}=35^{\circ}$}\DIFaddend . The CD-Bonn cross sections in contrast are generally smaller than the Paris\DIFdelbegin \DIFdel{/}\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend AV18 \DIFaddbegin \DIFadd{and WJC2 }\DIFaddend in this region.
...
\indent \DIFaddbegin \DIFadd{Moreover, in spite of the partial agreement between data and theory, at $p_{\mathrm{r}} \sim 0.55-1.0$ GeV/c, the data exhibits a smaller
fall-off compared to all models which was quantified by a linear fit to both the data and each of the PWIA calculations. A difference of at least
4.241 standard deviations was found which corresponds to a probability of $1.1\times 10^{-5}$ (very unlikely) that the observed discrepancy is due to a statistical fluctuation. }\\ 
...
%The main difference between the CD-Bonn and Paris/AV18 models is the use of Feynman amplitudes in covariant
%(original) form by the Bonn group as opposed to local (static) approximations of these amplitudes used by Paris/AV18
%groups to describe the NN potential.
...
Furthermore, the CD-Bonn model was found to be significantly different than the Paris\DIFdelbegin \DIFdel{or }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend AV18 \DIFaddbegin \DIFadd{or WJC2 }\DIFaddend models and was able to partially describe the data over a larger range in $p_{\mathrm{r}}$.
