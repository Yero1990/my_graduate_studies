...
For each spectrometer, a logical hardware signal was created from  the coincidence of hits in three of the four scintillator planes. The event trigger was the \DIFdelbegin \DIFdel{the }\DIFdelend coincidence of these two signals. \\
...
\indent For $^{1}\mathrm{H}(e,e'p)$, the corrected data yield was compared to SIMC calculations using \DIFdelbegin \DIFdel{J. }\DIFdelend Arrington's proton form factor parametrization \cite{PhysRevC.69.022201} to check the spectrometer acceptance
...
%DIF > \indent The systematic uncertainties due to our finite knowledge of the beam energy and spectrometer angle/momentum settings were determined point-to-point in ($\theta_{nq}, p_{\mathrm{r}}$) bins for each missing momentum setting,
%DIF > and added in quadrature for overlapping $p_{\mathrm{r}}$ bins.
\indent The systematic uncertainties \DIFdelbegin \DIFdel{due to our limited knowledge of the }\DIFdelend \DIFaddbegin \DIFadd{on the cross section due to systematic error on the absolute }\DIFaddend beam energy and spectrometer angle/momentum settings were
determined point-to-point in (\DIFdelbegin \DIFdel{$\theta_{nq}, p_{\mathrm{r}}$}\DIFdelend \DIFaddbegin \DIFadd{$\theta_{nq}$, $p_{\mathrm{r}}$}\DIFaddend ) bins for each missing momentum setting, and added in quadrature for overlapping $p_{\mathrm{r}}$ bins. 
...
\indent The data were radiatively corrected for each bin in ($\theta_{nq}, p_{\mathrm{r}}$) by multiplying the measured cross sections by the ratio of the calculated particle yield excluding and including radiative effects. The SIMC simulation code was used for these calculations with the Deuteron Model by \DIFdelbegin \DIFdel{J.M. }\DIFdelend Laget including FSI \cite{LAGET2005}.
...
over the kinematic bin. The systematic uncertainty associated with the radiative and bin-centering corrections was investigated using the \DIFdelbegin \DIFdel{J.M. }\DIFdelend Laget PWIA and 
...
\end{center}
\twocolumngrid
\noindent \DIFaddend FSI models but negligible effects on the cross sections were found. \DIFdelbegin %DIFDELCMD < \\
%DIFDELCMD < \indent %%%
...
Within the PWIA, $\sigma_{\mathrm{red}}$ corresponds to the \DIFdelbegin \DIFdel{proton momentum distribution inside }\DIFdelend \DIFaddbegin \DIFadd{PWIA cross section from the scattering of a proton in }\DIFaddend the deuteron. \\
...
for the CD-Bonn (magenta) and AV18 (green) potentials were performed by \DIFdelbegin \DIFdel{M. Sargsian \mbox{%DIFAUXCMD
\cite{PhysRevC.82.014612} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{Sargsian \mbox{%DIFAUXCMD
\cite{PhysRevC.82.014612} }\hspace{0pt}%DIFAUXCMD
within the GEA }\DIFaddend and those for the Paris potential (blue) were by \DIFdelbegin \DIFdel{J.M. Laget \mbox{%DIFAUXCMD
\cite{LAGET2005} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{Laget \mbox{%DIFAUXCMD
\cite{LAGET2005} }\hspace{0pt}%DIFAUXCMD
within the diagrammatic approach}\DIFaddend .\\
...
\indent Beyond \DIFdelbegin \DIFdel{$p_{\mathrm{r}}\sim300$ }\DIFdelend \DIFaddbegin \DIFadd{$p_{\mathrm{r}}\sim250$ }\DIFaddend MeV/c at $\theta_{nq}=35^{\circ}$ and $45^{\circ}$ (Figs. \hyperref[fig:fig1]{1(a), 1(b)}), the Paris and
AV18 models \DIFdelbegin \DIFdel{significantly }\DIFdelend \DIFaddbegin \DIFadd{increasingly }\DIFaddend differ from the CD-Bonn. In this region, the Paris/AV18 cross sections are dominated by the PWIA and \DIFdelbegin \DIFdel{within }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend good agreement up to $p_{\mathrm{r}}\sim700$ MeV/c. The CD-Bonn cross sections in contrast are generally smaller than the Paris/AV18 in this region.  In addition, for $\theta_{nq}=35^{\circ}$, they are dominated by the PWIA up to $p_{\mathrm{r}}\sim800$ MeV/c (Fig. \hyperref[fig:fig1]{1(a)})  while for $\theta_{nq}=45^{\circ}$  FSI start to contribute already above 600 MeV/c (Fig. \hyperref[fig:fig1]{1(b)}).\DIFdelbegin \DIFdel{The main }\DIFdelend \DIFaddbegin \\
\indent \DIFadd{The }\DIFaddend difference between the \DIFaddbegin \DIFadd{deuteron wave functions with }\DIFaddend CD-Bonn and Paris/AV18  \DIFdelbegin \DIFdel{models is 
the use of Feynman amplitudes in covariant (original) form by the 
Bonn group as opposed to local (static ) approximations of 
these amplitudes used by Paris /}\DIFdelend \DIFaddbegin \DIFadd{potentials is 
...
nucleon-neson-meson couplings are constrained to describe the NN scattering phase shifts
extracted from the data. The interaction potential represents the static limit of 
this potential. Paris and }\DIFaddend AV18 \DIFdelbegin \DIFdel{groups
}\DIFdelend \DIFaddbegin \DIFadd{potentials are phenomenological in which the 
Yukawa type interaction is introduced and parameters are fitted }\DIFaddend to describe the 
\DIFdelbegin \DIFdel{NN potential. The }\DIFdelend \DIFaddbegin \DIFadd{same NN scattering phase-shifts. The major difference between CD-Bonn and Paris/AV18 
...
%DIF > The main difference between the CD-Bonn and Paris/AV18 models is the use of Feynman amplitudes in covariant
%DIF > (original) form by the Bonn group as opposed to local (static) approximations of these amplitudes used by Paris/AV18
%DIF > groups to describe the NN potential.
The }\DIFaddend effect of these local approximations on the NN potential are shown in Fig. 2 of Ref. \cite{PhysRevC.63.024001}. \\
...
\DIFdelbegin \DIFdel{behaviour (smaller falloff}\DIFdelend \DIFaddbegin \DIFadd{behavior (smaller fall-off}\DIFaddend ) that overshadows any possibility of extracting the \DIFaddbegin \DIFadd{genuine }\DIFaddend momentum distributions.\\
...
At $p_{\mathrm{r}}>300$ MeV/c \DIFaddbegin \DIFadd{and $\theta_{nq}=75^{\circ}$}\DIFaddend , the data \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{were }\DIFaddend statistically limited as our focus was on the smaller recoil angles. The Hall A data, however, shows a reasonable agreement with the FSI from all models which gives us confidence in our understanding
