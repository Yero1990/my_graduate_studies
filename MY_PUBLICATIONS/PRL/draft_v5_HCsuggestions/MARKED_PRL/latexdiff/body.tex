...
cross sections using the charge-dependent Bonn (CD-Bonn) \cite{PhysRevC.63.024001}, Argonne $v_{18}$ (AV18) \cite{PhysRevC.51.38}, Paris \cite{PhysRevC.21.861} and WJC2 \cite{Gross_2007} NN potentials. The theoretical calculations
...
For the WJC2 (orange) potential, the calculations were done by Orden and Jeschonnek \cite{PhysRevC.90.064006} using a Bethe-Salpeter-like formalism for two-body bound states. The calculations
used two different form factor parametrizations (GKex05 \cite{PhysRevC.66.045501}, AMT \cite{PhysRevC.76.035205}) where a discrepancy of $\sim5.8-6.6 \%$ was found between the respective
WJC2 reduced cross sections. Figure \ref{fig:fig1} shows the results using only the WJC2 (GKex05) parametrization. \\
...
\indent Beyond $p_{\mathrm{r}}\sim250$ MeV/c at $\theta_{nq}=35^{\circ}$ and $45^{\circ}$ (Figs. \hyperref[fig:fig1]{1(a), 1(b)}), the Paris,
AV18 and WJC2 models increasingly differ from the CD-Bonn calculation. In this region, the Paris/AV18 cross sections are dominated by the PWIA and in good agreement up to $p_{\mathrm{r}}\sim700$ MeV/c whereas
the WJC2 PWIA falls off with a comparatively smaller cross section at $\theta_{nq}=35^{\circ}$. The CD-Bonn cross sections in contrast are generally smaller than the Paris, AV18 and WJC2 in this region.
...
\indent Moreover, in spite of the partial agreement between data and theory, at $p_{\mathrm{r}} \sim 0.55-1.0$ GeV/c, the data exhibits a smaller
...
\indent The difference between the deuteron wave functions with CD-Bonn and Paris/AV18 potentials is 
...
Furthermore, the CD-Bonn model was found to be significantly different than the Paris, AV18 or WJC2 models and was able to partially describe the data over a larger range in $p_{\mathrm{r}}$.
