...
over the kinematic bin. The systematic uncertainty associated with the radiative and bin-centering corrections was investigated using the Laget PWIA and \DIFaddbegin \DIFadd{FSI models but negligible effects on the cross sections 
}\DIFaddend \onecolumngrid
\begin{center}
\begin{figure}
...
\end{center}
\twocolumngrid
\noindent \DIFdelbegin \DIFdel{FSI models but negligible effects on the cross sections }\DIFdelend were found. The experimental and theoretical reduced cross sections were extracted and are defined as follows:
\begin{equation}
...
cross sections }\DIFdelend \DIFaddbegin \DIFadd{calculations }\DIFaddend using wave functions determined from the charge-dependent Bonn (CD-Bonn) \cite{PhysRevC.63.024001}, Argonne $v_{18}$ (AV18) \cite{PhysRevC.51.38}, Paris \cite{PhysRevC.21.861} and WJC2 \cite{Gross_2007} NN potentials. The theoretical calculations
...
For the WJC2 (orange) potential, the calculations were carried out by \DIFaddbegin \DIFadd{Ford, }\DIFaddend Jeschonnek and Van Orden \cite{PhysRevC.90.064006} using a Bethe-Salpeter-like formalism for two-body bound states, which will be labeled JVO.
\DIFdelbegin \DIFdel{The calculations
used two different form factor }\DIFdelend %DIF > -----Mention the fofa parametrizations used in each of the calculations------
\DIFaddbegin \DIFadd{The MS calculations used the FF parametrization of Ref. \mbox{%DIFAUXCMD
...
for the proton electric FF parametrization. The JVO calculations used two different FF }\DIFaddend parametrizations (GKex05 \cite{PhysRevC.66.045501}, AMT \cite{PhysRevC.76.035205}) where a difference of $\sim5.8-6.6 \%$ was found between the respective
JVO reduced cross sections. Figure \ref{fig:fig1} shows the results using only the GKex05 parametrization. \\
\indent The difference between the deuteron wave functions with CD-Bonn\DIFdelbegin \DIFdel{and Paris/}\DIFdelend \DIFaddbegin \DIFadd{, Paris, }\DIFaddend AV18 \DIFaddbegin \DIFadd{and WJC2 }\DIFaddend potentials is 
how the NN potential is modeled based on the empirical NN scattering data.
The CD-Bonn model is based on the One-Boson-Exchange \DIFaddbegin \DIFadd{Potential (OBEP) }\DIFaddend approach in which the 
...
this potential. \DIFaddbegin \DIFadd{In contrast, the WJC2 is a OBEP derived within the Covariant Spectator Theory (CST) \mbox{%DIFAUXCMD
...
The }\DIFaddend Paris and AV18 are purely phenomenological potentials where a 
Yukawa type interaction is introduced and parameters are fitted to describe the 
same NN scattering phase-shifts. The major difference between the CD-Bonn and Paris/AV18\DIFaddbegin \DIFadd{/WJC2 
}\DIFaddend potentials is that the former predicts a much softer repulsive interaction at short distance which 
...
\indent \DIFdelbegin \DIFdel{Between data and theory, for }\DIFdelend \DIFaddbegin \DIFadd{For }\DIFaddend recoil momenta $p_{\mathrm{r}} \sim 0.55-1.0$ GeV/c \DIFaddbegin \DIFadd{(Figs. }\hyperref[fig:fig1]{1(a), 1(b)}\DIFadd{)}\DIFaddend , all models exhibit a
...
thank Misak Sargsian\DIFaddbegin \DIFadd{, J.M. Laget}\DIFaddend , Sabine Jeschonnek and J.W. Van Orden for providing the theoretical calculations as well as the useful
