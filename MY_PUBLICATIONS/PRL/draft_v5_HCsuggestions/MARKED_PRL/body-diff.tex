...
For each spectrometer, a logical \DIFdelbegin \DIFdel{hardware }\DIFdelend signal was created from  the coincidence of hits in three of the four scintillator planes. The event trigger was the coincidence of these two signals. \\
...
\indent The kinematics of the recoiling neutron was reconstructed using energy-momentum conservation. The recoil momentum is defined as $\vec{p_{\mathrm{r}}} = \vec{q} - \vec{p_{\mathrm{f}}}$\DIFaddbegin \DIFadd{,
...
\indent For $^{1}\mathrm{H}(e,e'p)$, the corrected data yield was compared to SIMC calculations using Arrington's proton form factor \DIFaddbegin \DIFadd{(FF) }\DIFaddend parametrization \cite{PhysRevC.69.022201} to check the spectrometer acceptance
...
determined point-to-point in ($\theta_{nq}$, $p_{\mathrm{r}}$) bins for each missing momentum setting, and added in quadrature for overlapping \DIFdelbegin \DIFdel{pr }\DIFdelend \DIFaddbegin \DIFadd{$p_{\mathrm{r}}$ }\DIFaddend bins. 
...
to the statistical uncertainty (\DIFdelbegin \DIFdel{25-30}\DIFdelend \DIFaddbegin \DIFadd{20-30}\DIFaddend $\%$ on average) to obtain the final uncertainty in the cross section. \\
...
\caption{\DIFaddFL{The reduced cross sections $\sigma_{\mathrm{red}}(p_{\mathrm{r}})$ as a function of neutron recoil momentum $p_{\mathrm{r}}$ are shown in (a)-(c) for recoil angles $\theta_{nq}=35^{\circ}, 45^{\circ}$ and $75^{\circ}$, respectively,
with a bin width of $\pm 5^{\circ}$. The data is compared to the previous Hall A experiment (red square) results \mbox{%DIFAUXCMD
\cite{PhysRevLett.107.262501} }\hspace{0pt}%DIFAUXCMD
as well as the theoretical reduced cross sections using the Paris (blue),
AV18 (green), CD-Bonn (magenta) and WJC2 (orange) NN potentials.}}
\label{fig:fig1}
\end{figure}
\end{center}
\twocolumngrid
\noindent \DIFaddend were found. \DIFdelbegin %DIFDELCMD < \\
%DIFDELCMD < \indent %%%
\DIFdelend The experimental and theoretical reduced cross sections were extracted and are defined as follows:
\DIFdelbegin %DIFDELCMD < \onecolumngrid
%DIFDELCMD < \begin{center}
%DIFDELCMD < \begin{figure}
%DIFDELCMD < \includegraphics[scale=0.46]{prl_plots/PRL_plot1.pdf}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{The reduced cross sections $\sigma_{\mathrm{red}}(p_{\mathrm{r}})$ as a function of neutron recoil momentum $p_{\mathrm{r}}$ are shown in (a)-(c) for recoil angles $\theta_{nq}=35^{\circ}, 45^{\circ}$ and $75^{\circ}$, respectively,
with a bin width of $\pm 5^{\circ}$. The data is compared to the previous Hall A experiment (red square) results \mbox{%DIFAUXCMD
\cite{PhysRevLett.107.262501} }\hspace{0pt}%DIFAUXCMD
as well as the theoretical reduced cross sections using the Paris(blue),
...
de Forest \cite{DEFOREST1983} electron-proton off-shell cross section calculated using the \DIFdelbegin \DIFdel{form factor }\DIFdelend \DIFaddbegin \DIFadd{FF }\DIFaddend parametrization of Ref. \cite{PhysRevC.69.022201}.
...
$p_{\mathrm{r}}$ for three recoil angle settings at $Q^{2}=4.5\pm0.5$ (GeV/c)$^{2}$. For the two highest \DIFaddbegin \DIFadd{momentum }\DIFaddend settings ($p_{\mathrm{r}}=580, 750$ MeV/c), a weighted average of the cross sections were taken in the overlapping regions of $p_{\mathrm{r}}$. The results from the previous experiment \cite{PhysRevLett.107.262501} at a $Q^{2}=3.5\pm0.25$ (GeV/c)$^{2}$ are plotted as well (red square). The good agreement between the Hall A and C data at lower $p_{\mathrm{r}}$
gives us confidence in the measurements made at higher missing momentum settings for which no previous data exist.
The data are compared to theoretical \DIFdelbegin \DIFdel{reduced
cross sections using }\DIFdelend \DIFaddbegin \DIFadd{calculations using wave functions determined from }\DIFaddend the charge-dependent Bonn (CD-Bonn) \cite{PhysRevC.63.024001}, Argonne $v_{18}$ (AV18) \cite{PhysRevC.51.38}\DIFdelbegin \DIFdel{and Paris \mbox{%DIFAUXCMD
\cite{PhysRevC.21.861} }\hspace{0pt}%DIFAUXCMD
NN-potentials}\DIFdelend \DIFaddbegin \DIFadd{, Paris \mbox{%DIFAUXCMD
\cite{PhysRevC.21.861} }\hspace{0pt}%DIFAUXCMD
and WJC2 \mbox{%DIFAUXCMD
\cite{Gross_2007} }\hspace{0pt}%DIFAUXCMD
NN potentials}\DIFaddend . The theoretical calculations
for the CD-Bonn (magenta) and AV18 (green) potentials were performed by Sargsian \cite{PhysRevC.82.014612} \DIFaddbegin \DIFadd{within the GEA, referred to as MS, }\DIFaddend and those for the Paris potential (blue) were by Laget \cite{LAGET2005} \DIFdelbegin \DIFdel{.}\DIFdelend \DIFaddbegin \DIFadd{within the diagrammatic approach, referred to as JML.
For the WJC2 (orange) potential, the calculations were carried out by Ford, Jeschonnek and Van Orden \mbox{%DIFAUXCMD
\cite{PhysRevC.90.064006} }\hspace{0pt}%DIFAUXCMD
using a Bethe-Salpeter-like formalism for two-body bound states, which will be labeled JVO.
%DIF > -----Mention the fofa parametrizations used in each of the calculations------
The MS calculations used the FF parametrization of Ref. \mbox{%DIFAUXCMD
\cite{PhysRevC.70.068202} }\hspace{0pt}%DIFAUXCMD
while the JML calculations used the conventional dipole parametrization for the proton and neutron magnetic FF, the Galster \mbox{%DIFAUXCMD
\cite{Galster:1971} }\hspace{0pt}%DIFAUXCMD
parametrization
for the neutron electric FF, and the results from the Hall A experiment of Ref. \mbox{%DIFAUXCMD
\cite{PhysRevLett.88.092301} }\hspace{0pt}%DIFAUXCMD
for the proton electric FF parametrization. The JVO calculations used two different FF parametrizations (GKex05 \mbox{%DIFAUXCMD
\cite{PhysRevC.66.045501}}\hspace{0pt}%DIFAUXCMD
, AMT \mbox{%DIFAUXCMD
\cite{PhysRevC.76.035205}}\hspace{0pt}%DIFAUXCMD
) where a difference of $\sim5.8-6.6 \%$ was found between the respective
JVO reduced cross sections. Figure \ref{fig:fig1} shows the results using only the GKex05 parametrization. }\DIFaddend \\
\indent \DIFaddbegin \DIFadd{The difference between the deuteron wave functions with CD-Bonn, Paris, AV18 and WJC2 potentials is 
...
this potential. In contrast, the WJC2 is a OBEP derived within the Covariant Spectator Theory (CST) \mbox{%DIFAUXCMD
\cite{PhysRev.186.1448, PhysRevD.10.223, PhysRevC.26.2203, PhysRevC.26.2226}
}\hspace{0pt}%DIFAUXCMD
which require comparatively few parameters while still producing a high-precision fit to the NN scattering data.
...
The effect of these local approximations on the NN potential are shown in Fig. 2 of Ref. \mbox{%DIFAUXCMD
\cite{PhysRevC.63.024001}}\hspace{0pt}%DIFAUXCMD
.}\\
\indent \DIFaddend For all recoil angles shown in Fig. \hyperref[fig:fig1]{1} at recoil momenta \DIFdelbegin \DIFdel{$p_{\mathrm{r}}\leq300$ }\DIFdelend \DIFaddbegin \DIFadd{$p_{\mathrm{r}}\leq250$ }\DIFaddend MeV/c, the cross sections are well reproduced by all models when FSI are included.
The agreement at \DIFdelbegin \DIFdel{$p_{\mathrm{r}}\leq300$ }\DIFdelend \DIFaddbegin \DIFadd{$p_{\mathrm{r}}\leq250$ }\DIFaddend MeV/c can be understood from the fact that this region corresponds to the long-range part of the NN potential where the One Pion Exchange Potential (OPEP)
is well known and common to all modern potentials. \\
\indent Beyond \DIFdelbegin \DIFdel{$p_{\mathrm{r}}\sim300$ }\DIFdelend \DIFaddbegin \DIFadd{$p_{\mathrm{r}}\sim250$ }\DIFaddend MeV/c at $\theta_{nq}=35^{\circ}$ and $45^{\circ}$ (Figs. \hyperref[fig:fig1]{1(a), 1(b)}), the \DIFdelbegin \DIFdel{Paris and
}\DIFdelend \DIFaddbegin \DIFadd{JML,
MS }\DIFaddend AV18 \DIFdelbegin \DIFdel{models significantly }\DIFdelend \DIFaddbegin \DIFadd{and JVO models increasingly }\DIFaddend differ from the \DIFaddbegin \DIFadd{MS }\DIFaddend CD-Bonn \DIFaddbegin \DIFadd{calculation}\DIFaddend . In this region, the \DIFdelbegin \DIFdel{Paris/}\DIFdelend \DIFaddbegin \DIFadd{JML and MS }\DIFaddend AV18 cross sections are dominated by the PWIA and \DIFdelbegin \DIFdel{within }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend good agreement up to $p_{\mathrm{r}}\sim700$ MeV/c \DIFdelbegin \DIFdel{. The }\DIFdelend \DIFaddbegin \DIFadd{whereas
the JVO PWIA falls off with a comparatively smaller cross section at $\theta_{nq}=35^{\circ}$. The MS }\DIFaddend CD-Bonn cross sections in contrast are generally smaller than the \DIFdelbegin \DIFdel{Paris/}\DIFdelend \DIFaddbegin \DIFadd{JML, MS }\DIFaddend AV18 \DIFaddbegin \DIFadd{and JVO }\DIFaddend in this region.
In addition, for $\theta_{nq}=35^{\circ}$, they are dominated by the PWIA up to $p_{\mathrm{r}}\sim800$ MeV/c
(Fig. \hyperref[fig:fig1]{1(a)})  while for $\theta_{nq}=45^{\circ}$  FSI start to contribute already above 600 MeV/c (Fig. \hyperref[fig:fig1]{1(b)}).\DIFdelbegin \DIFdel{The main difference between the CD-Bonn and Paris}\DIFdelend \DIFaddbegin \\
\indent \DIFadd{For recoil momenta $p_{\mathrm{r}} \sim 0.55-1.0$ GeV}\DIFaddend /\DIFdelbegin \DIFdel{AV18 models is the use of Feynman amplitudes in covariant (original)form by the Bonn group as opposed to local (static) approximations of these amplitudes used by Paris/AV18 groups
to describe the NN potential. The effect of these local approximations on the NN potential are shown in Fig. 2 of Ref. \mbox{%DIFAUXCMD
\cite{PhysRevC.63.024001}}\hspace{0pt}%DIFAUXCMD
. }\DIFdelend \DIFaddbegin \DIFadd{c (Figs. }\hyperref[fig:fig1]{1(a), 1(b)}\DIFadd{), all models exhibit a
...
4.2 standard deviations was found between the data and theory slopes which corresponds to a probability $\leq 1.1\times 10^{-5}$ (very unlikely) that the observed discrepancy is due to a statistical fluctuation. }\DIFaddend \\ 
...
\DIFdelbegin \DIFdel{behaviour }\DIFdelend \DIFaddbegin \DIFadd{behavior }\DIFaddend (smaller fall-off) that overshadows any possibility of extracting the \DIFaddbegin \DIFadd{approximate }\DIFaddend momentum distributions.\\
...
\indent For $\theta_{nq}=35^{\circ}$ and $45^{\circ}$ (Figs. \hyperref[fig:fig2]{2(a),(b)}), the data are best described by the \DIFaddbegin \DIFadd{MS }\DIFaddend CD-Bonn PWIA calculation for recoil momenta up
to $p_{\mathrm{r}}\sim700$ MeV/c and $\sim600$ MeV/c, respectively. Furthermore, the agreement between the Halls A and C data \DIFdelbegin \DIFdel{solidifies }\DIFdelend \DIFaddbegin \DIFadd{confirm }\DIFaddend the Hall A approach of selecting a kinematic
region where recoil angles are small and FSI are reduced. \\
\indent At larger recoil momenta, where the ratio \DIFdelbegin \DIFdel{is }\DIFdelend $R>1$ and increasing \DIFaddbegin \DIFadd{with $p_{\mathrm{r}}$}\DIFaddend , for $\theta_{nq}=35^{\circ}$ FSI start to dominate for
missing momenta typically above 800 MeV/c for the \DIFaddbegin \DIFadd{MS }\DIFaddend CD-Bonn
...
The dashed reference (magenta) line refers to \DIFaddbeginFL \DIFaddFL{MS }\DIFaddendFL CD-Bonn PWIA calculation (or momentum distribution) by which the data and all models are divided.}
\label{fig:fig2}
\end{figure}
calculation while the other models predict still relatively small FSI below 900 MeV/c.
At $\theta_{nq}=45^{\circ}$\DIFaddbegin \DIFadd{, }\DIFaddend the FSI dominance starts earlier for all models above 800 MeV/c and for the \DIFaddbegin \DIFadd{MS }\DIFaddend CD-Bonn based calculation above 600 MeV/c. \\
...
At $p_{\mathrm{r}}>300$ MeV/c \DIFaddbegin \DIFadd{and $\theta_{nq}=75^{\circ}$}\DIFaddend , the data were statistically limited as our focus was on the smaller recoil angles. The Hall A data, however, shows a reasonable agreement with the FSI from all models which gives us confidence in our understanding
of FSI at the smaller recoil angles. \\
\indent \DIFdelbegin \DIFdel{This }\DIFdelend \DIFaddbegin \DIFadd{To summarize, this }\DIFaddend experiment extended the previous Hall A cross section measurements on the $^{2}\mathrm{H}(e,e'p)n$ reaction to 
...
short range part of the deuteron \DIFdelbegin \DIFdel{wavefunction}\DIFdelend \DIFaddbegin \DIFadd{wave function}\DIFaddend . The experimental reduced cross sections were extracted and found to be in good agreement with the Hall A data.
Furthermore, the \DIFaddbegin \DIFadd{MS }\DIFaddend CD-Bonn model was found to be significantly different than the \DIFdelbegin \DIFdel{Paris or }\DIFdelend \DIFaddbegin \DIFadd{JML, MS }\DIFaddend AV18 \DIFaddbegin \DIFadd{or JVO }\DIFaddend models and was able to partially describe the data over a larger range in $p_{\mathrm{r}}$.
At higher missing momenta \DIFaddbegin \DIFadd{($p_{\mathrm{r}}>700$ MeV/c)}\DIFaddend , however, all models were unable to describe the data. 
\DIFdelbegin %DIFDELCMD < \\
%DIFDELCMD < \indent %%%
\DIFdel{In conclusion, additional }\DIFdelend \DIFaddbegin \DIFadd{Additional }\DIFaddend measurements of the $^{2}\mathrm{H}(e,e'p)n$ would be required to reduce the statistical uncertainties in this very high missing
momentum region ($p_{\mathrm{r}}>500$ MeV/c) to better understand the large deviations observed between the different models and data.\DIFaddbegin \\
...
to the equipment for the Hall C upgrade, making all four commissioning experiments possible. \DIFaddbegin \DIFadd{We would also like to
...
discussions we had on this topic. }\DIFaddend This work was supported in part by the U.S. Department of Energy (DOE), Office of Science, Office of Nuclear Physics
under grant No. DE-SC0013620 and contract DE-AC05-06OR23177, the Nuclear Regulatory Commission (NRC) Fellowship
under grant No. NRC-HQ-84-14-G-0040\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend the Doctoral Evidence Acquisition (DEA) Fellowship \DIFaddbegin \DIFadd{and the Natural Sciences and Engineering Research Council of Canada (NSERC)}\DIFaddend .
