...
%DIF DEL ms_revised_v3.tex   Wed Oct 14 19:57:07 2020
%DIF ADD ms_revised_v5.tex   Tue Oct 20 00:22:24 2020
\pdfoutput=1
...
\begin{abstract}
  ...
  theoretical calculations, where we observe a significant discrepancy for \DIFdelbegin \DIFdel{missing }\DIFdelend \DIFaddbegin \DIFadd{recoil }\DIFaddend momenta $p_{\mathrm{r}}>700$ MeV/c. 
\end{abstract}
...
carried out at Jefferson Lab (JLab) \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{PhysRevLett.113.022501,PhysRevLett.122.172502,PhysRevLett.124.212501} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend and Brookhaven National Lab (BNL) \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{PhysRevLett.97.162504}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . In these experiments missing momenta up to $\sim1$ GeV/c have been probed and missing momentum distributions
...
(Bjorken $x_{\mathrm{Bj}}\sim1$) where the cross section is dominated by final state interactions. Due to FSI effects, the measurement of \DIFaddbegin \DIFadd{a }\DIFaddend certain missing momentum does not yet guarantee that the initial
...
\indent At large $Q^{2}$, FSI can be described by the Generalized Eikonal Approximation (GEA) \cite{Sargsian_2001,PhysRevC.56.1124,sargsian_2015}, which predicts a strong dependence of FSI on neutron recoil angles $\theta_{nq}$ \DIFaddbegin \DIFadd{(relative angles between recoil momenta $\vec{p}_{\mathrm{r}}$ and 3-momentum transfers $\vec{q}$)}\DIFaddend .
...
experiments determined that the cross sections for fixed \DIFdelbegin \DIFdel{missing }\DIFdelend \DIFaddbegin \DIFadd{recoil }\DIFaddend momenta indeed exhibited a strong angular dependence with $\theta_{nq}$, peaking
...
\indent We measured three central \DIFdelbegin \DIFdel{missing }\DIFdelend \DIFaddbegin \DIFadd{recoil }\DIFaddend momentum settings: $p_{\mathrm{r}}=80, 580$ and $750$ MeV/c. At each of these settings, the electron arm (SHMS) was fixed and the proton arm (HMS) was rotated from smaller to larger angles corresponding to
the lower and higher \DIFdelbegin \DIFdel{missing }\DIFdelend \DIFaddbegin \DIFadd{recoil }\DIFaddend momentum settings, respectively. At these kinematic settings, the 3-momentum transfer covered a range of $2.4\lesssim|\vec{q}|\lesssim3.2$ GeV/c, which is more than twice the highest neutron recoil momentum
...
simulation program, SIMC \cite{PhysRevC.64.054610}\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{cyero_phdthesis}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend . Additional $^{1}\mathrm{H}(e,e'p)$ data were also taken at three other kinematic settings that covered the SHMS momentum acceptance range for the deuteron and were used for spectrometer optics optimization, 
...
target boiling inefficiency ($4.2 \%$). The values in parentheses were averaged over all \DIFdelbegin \DIFdel{missing }\DIFdelend \DIFaddbegin \DIFadd{recoil }\DIFaddend momentum settings. \\
...
the systematic uncertainties due to proton loss in HMS ($0.49 \%$), total live time ($3.0 \%$)\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend total charge ($2.0\%$), \DIFaddbegin \DIFadd{target wall contributions ($\leq2.9\%$) and spectrometer acceptance ($1.4\%$),  }\DIFaddend which were the same for every setting,
to define the overall normalization uncertainty (\DIFdelbegin \DIFdel{$\leq$ 4.2$\%$}\DIFdelend \DIFaddbegin \DIFadd{$\leq5.3\%$}\DIFaddend ).\\
...
determined point-to-point in ($\theta_{nq}$, $p_{\mathrm{r}}$) bins for each \DIFdelbegin \DIFdel{missing }\DIFdelend \DIFaddbegin \DIFadd{recoil }\DIFaddend momentum setting, and added in quadrature for overlapping $p_{\mathrm{r}}$ bins. 
For $\theta_{nq}= 35^{\circ}, 45^{\circ}$ and $75^{\circ}$ (presented in this Letter) the overall kinematic uncertainty varied below 6.5$\%$.
The total uncertainty was defined as the quadrature sum of the normalization ($\leq$ \DIFdelbegin \DIFdel{4.2}\DIFdelend \DIFaddbegin \DIFadd{5.3}\DIFaddend $\%$), kinematic ($\leq$ 6.5$\%$) and statistical ($\sim20-30\%$ on average) uncertainties.\\
\begin{figure*}[t]
\centering        
...
($E_{\mathrm{f}}, p_{\mathrm{f}}$) are the final proton energy and momentum, respectively, \DIFdelbegin \DIFdel{$f_{\mathrm{rec}}\equiv \frac{2p_{\mathrm{f}}^{2}E_{\mathrm{r}}}{2p_{\mathrm{f}}^{2}E_{\mathrm{r}} - (q^{2} - (p_{\mathrm{f}}^{2} + p_{\mathrm{r}}^{2}))E_{\mathrm{f}}}$
}\DIFdelend \DIFaddbegin \DIFadd{$f_{\mathrm{rec}}$ %DIF > \equiv \frac{2p_{\mathrm{f}}^{2}E_{\mathrm{r}}}{2p_{\mathrm{f}}^{2}E_{\mathrm{r}} - (q^{2} - (p_{\mathrm{f}}^{2} + p_{\mathrm{r}}^{2}))E_{\mathrm{f}}}$
}\DIFaddend is a recoil factor \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{cyero_phdthesis} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend obtained by integrating over the \DIFdelbegin \DIFdel{missing }\DIFdelend \DIFaddbegin \DIFadd{binding }\DIFaddend energy of the bound state in the 6-fold differential cross section \DIFdelbegin \DIFdel{where $E_{\mathrm{r}}$ is the neutron recoil energy, }\DIFdelend and $\sigma_{\mathrm{cc1}}$ is the
...
The calculations use different FF parametrizations which can lead to a \DIFdelbegin \DIFdel{6}\DIFdelend \DIFaddbegin \DIFadd{$\sim$5.8 -6.6}\DIFaddend $\%$ \DIFdelbegin \DIFdel{change }\DIFdelend \DIFaddbegin \DIFadd{variation }\DIFaddend of the theortical cross section.\\
...
At the higher \DIFdelbegin \DIFdel{missing }\DIFdelend \DIFaddbegin \DIFadd{recoil }\DIFaddend momenta provided by this experiment ($p_{\mathrm{r}}>700$ MeV/c), however, all models were unable to describe the data\DIFaddbegin \DIFadd{,
...
experimental data that probe the high momentum region of the $np$ system in the most direct way possible}\DIFaddend .
Additional measurements of the $^{2}\mathrm{H}(e,e'p)n$ would be required to reduce the statistical uncertainties in this very high \DIFdelbegin \DIFdel{missing
}\DIFdelend \DIFaddbegin \DIFadd{recoil
}\DIFaddend momentum region ($p_{\mathrm{r}}>500$ MeV/c) to better understand the large deviations observed between the different models and data.\\
