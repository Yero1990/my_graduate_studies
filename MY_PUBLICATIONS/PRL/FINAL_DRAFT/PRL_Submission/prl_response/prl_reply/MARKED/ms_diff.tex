...
%DIF DEL ms.tex           Thu Oct  8 16:13:34 2020
%DIF ADD ms_revised.tex   Thu Oct  8 16:13:22 2020
\pdfoutput=1
...
practically unexplored and not well understood. \DIFdelbegin \DIFdel{At such small inter-nucleon distances, the nucleon-nucleon ($NN$) interaction is expected to become repulsive and the interacting
nucleons begin to overlap. The dynamics in this short distance region are directly related to the dynamics of two-nucleon short range }\DIFdelend \DIFaddbegin \DIFadd{Understanding the high momentum structure of the proton-neutron ($pn$) system is highly important for nuclear physics due to the observed 
dominance of short-range }\DIFaddend correlations (SRC) \DIFdelbegin \DIFdel{observed in $A>2$ nuclei \mbox{%DIFAUXCMD
\cite{PhysRevC.68.014313,PhysRevLett.96.082501,PhysRevLett.99.072501,Fomin_2017,Hen2014,RevModPhys.89.045002}}\hspace{0pt}%DIFAUXCMD
. Short-range studies of the deuteron are also important in determining whether, or to what extent, the description of nuclei in terms of nucleon }\DIFdelend \DIFaddbegin \DIFadd{in nuclei at nucleon momenta above the fermi momentum. This dominance has been well established by a series of recent experiments
carried out at Jefferson Lab (JLab) and Berkeley National Lab (BNL). In these experiments nucleon momenta up to $p_{\mathrm{r}}\sim1$ GeV}\DIFaddend /\DIFdelbegin \DIFdel{meson degrees of freedom is still valid, before
having to include explicit quark degrees of freedom, an issue of fundamental importance in nuclear physics \mbox{%DIFAUXCMD
\cite{sargsian_2015}}\hspace{0pt}%DIFAUXCMD
.
As of the present time, there are only a few nuclear physics experiments for
which a transition between nucleonic to quark degrees of freedom has been observed \mbox{%DIFAUXCMD
\cite{PhysRevLett.81.4576,PhysRevLett.87.102302,PhysRevC.66.042201}}\hspace{0pt}%DIFAUXCMD
.
However, in contrast to SRCs, they were related mostly to the final state structure of nuclei rather than to its initial state (see, e.g., Ref.\mbox{%DIFAUXCMD
\cite{PhysRevLett.84.3045}}\hspace{0pt}%DIFAUXCMD
).}\DIFdelend \DIFaddbegin \DIFadd{c have been probed and recoil momentum distributions
have been compared to the high momentum part of theoretical deuteron momentum distributions \mbox{%DIFAUXCMD
\cite{PhysRevLett.97.162504,PhysRevLett.113.022501,PhysRevLett.122.172502,PhysRevLett.124.212501}}\hspace{0pt}%DIFAUXCMD
.
Recoil momenta up to $p_{\mathrm{r}}\sim1$ GeV/c have also been probed in a $^{3}\mathrm{He}(e,e'p)$ experiment \mbox{%DIFAUXCMD
\cite{PhysRevLett.94.082305,PhysRevLett.94.192302} }\hspace{0pt}%DIFAUXCMD
but at a relatively low momentum transfer of 1.5 (GeV/c)$^{2}$ and at a kinematic region
(Bjorken $x_{\mathrm{Bj}}\sim1$) where the cross section is dominated by final state interactions.}\DIFaddend \\
%DIF > ===Original introguction (replaced by above intro, suggested by W. Boeglin, in response to referee. comments)====
%DIF > At such small inter-nucleon distances, the nucleon-nucleon ($NN$) interaction is expected to become repulsive and the interacting
%DIF > nucleons begin to overlap. The dynamics in this short distance region are directly related to the dynamics of two-nucleon short range correlations (SRC) observed in $A>2$ nuclei \cite{PhysRevC.68.014313,PhysRevLett.96.082501,PhysRevLett.99.072501,Fomin_2017,Hen2014,RevModPhys.89.045002}.
%DIF > Short-range studies of the deuteron are also important in determining whether, or to what extent, the description of nuclei in terms of nucleon/meson degrees of freedom is still valid, before
%DIF > having to include explicit quark degrees of freedom, an issue of fundamental importance in nuclear physics \cite{sargsian_2015}. As of the present time, there are only a few nuclear physics experiments for
%DIF > which a transition between nucleonic to quark degrees of freedom has been observed \cite{PhysRevLett.81.4576,PhysRevLett.87.102302,PhysRevC.66.042201}.
%DIF > However, in contrast to SRCs, they were related mostly to the final state structure of nuclei rather than to its initial state (see, e.g., Ref.\cite{PhysRevLett.84.3045}).\\
\indent The most direct way to study the short range structure of the deuteron wave function \DIFdelbegin \DIFdel{(or equivalently, its high momentum component) }\DIFdelend %DIF > (or equivalently, its high momentum component)
...
by the new SHMS and the existing High Momentum Spectrometer (HMS), respectively. The beam currents delivered by the accelerator ranged between 45-60 $\mu$A and the beam was rastered over a 2x2 mm$^{2}$ area to reduce the effects of localized boiling on the cryogenic targets\DIFdelbegin \DIFdel{(hydrogen and deuterium)}\DIFdelend .\\ %DIF > (hydrogen and deuterium).\\
\indent Both Hall C spectrometers have similar standard detector packages \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{cyero_phdthesis}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , each with four scintillator planes \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{hodo_techreport} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend used for triggering, a pair of drift chambers \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{dc_techreport} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend used for tracking, and a calorimeter \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Mkrtchyan_2013} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend and gas \u{C}erenkov \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Li_Wenliang_mthesis,ngc_techreport} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend used for electron identification.
For each spectrometer, a logic signal was created from  the coincidence of hits in \DIFaddbegin \DIFadd{at least }\DIFaddend three of the four scintillator planes. The event trigger was the coincidence of these two signals. \\
...
simulation program, SIMC \cite{PhysRevC.64.054610} \DIFaddbegin \DIFadd{(see Supplemental Material)}\DIFaddend . Additional $^{1}\mathrm{H}(e,e'p)$ data were also taken at three other kinematic settings that covered the SHMS momentum acceptance range for the deuteron and were used for spectrometer optics optimization, 
...
originated from the same reaction vertex at the target \DIFaddbegin \DIFadd{(see Supplemental Material)}\DIFaddend . \\
...
were as follows\DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{cyero_phdthesis}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend : tracking efficiencies ($98.9 \%$-HMS, $96.4 \%$-SHMS), total live time ($92.3 \%$), proton loss inefficiency due to nuclear interactions in the HMS ($4.7 \%$) \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{cyero_phdthesis} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend and
target boiling inefficiency ($4.2 \%$)\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{cyero_phdthesis}}\hspace{0pt}%DIFAUXCMD
...
contributions from normalization uncertainties for each setting were determined to be (on average)\DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{cyero_phdthesis}}\hspace{0pt}%DIFAUXCMD
...
\caption{(Color online). The ratio $R(p_{\mathrm{r}}$) is shown in (a)-(c) for $\theta_{nq}=35^{\circ}, 45^{\circ}$ and $75^{\circ}$, respectively, each with a bin width of $\pm 5^{\circ}$.
  ...
  \DIFaddbeginFL \DIFaddFL{Insets: Blowup of the subfigures below $p_{\mathrm{r}}=$0.7 GeV/c.}\DIFaddendFL }
\label{fig:fig2}
\end{figure}
...
%DIF > ---include references that are not mentioned in the main text
%DIF > ---but are included in the supplemental material
%DIF > ---cite supp. material as follows:  (See Supplemental Material) after description of the topic being discussed.
%DIF > \nocite{COSY}
