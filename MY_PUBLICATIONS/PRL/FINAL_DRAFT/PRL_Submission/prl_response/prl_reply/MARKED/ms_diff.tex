...
%DIF DEL ms_revised_v2.tex   Tue Oct 13 19:01:11 2020
%DIF ADD ms_revised_v3.tex   Wed Oct 14 19:31:03 2020
\pdfoutput=1
...
simulation program, SIMC \cite{PhysRevC.64.054610}\DIFdelbegin \DIFdel{(see Supplemental Material)}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{cyero_phdthesis}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . Additional $^{1}\mathrm{H}(e,e'p)$ data were also taken at three other kinematic settings that covered the SHMS momentum acceptance range for the deuteron and were used for spectrometer optics optimization, 
momentum calibration and the determination of the spectrometer offsets and kinematic uncertainties \cite{cyero_phdthesis}.\\
%DIF > \indent The kinematics of the recoiling neutron was reconstructed using energy-momentum conservation. The recoil momentum is defined as $\vec{p_{\mathrm{r}}} = \vec{q} - \vec{p_{\mathrm{f}}}$,
%DIF > and the nuclear binding (or ``missing'') energy as $E_{\mathrm{m}} = \omega - T_{p} - T_{\mathrm{r}}$, where $\vec{p_{\mathrm{f}}}$ is the final proton momentum, $\vec{q}$ is the
%DIF > 3-momentum transfer and $T_{p}$ is the final proton kinetic energy. The recoil particle kinetic energy, $T_{\mathrm{r}}$, is calculated from the electron and proton
%DIF > 4-momentum vectors assuming an exclusive three-body final state with a recoiling neutron.\\
\indent \DIFdelbegin \DIFdel{The kinematics of the recoiling neutron was reconstructed using energy-momentum conservation. The recoil momentum is defined as $\vec{p_{\mathrm{r}}} = \vec{q} - \vec{p_{\mathrm{f}}}$,
...
4-momentum vectors assuming an exclusive three-body final state with a recoiling neutron.}%DIFDELCMD < \\
%DIFDELCMD < \indent %%%
\DIFdelend Identical event selection criteria were used for the hydrogen and deuteron data. The criteria were determined by making standard cuts on the spectrometer momentum fraction ($\delta$) to select a region for which the reconstruction optics
are well known,  a cut to restrict the HMS solid angle acceptance to events that passed directly through the collimator and not by re-scattering from the collimator edges, a \DIFdelbegin \DIFdel{``missing''
}\DIFdelend \DIFaddbegin \DIFadd{reconstructed binding
}\DIFaddend energy cut (peak $\sim$ 2.22 MeV for the deuteron) to select true $^{2}\mathrm{H}(e,e'p)n$ coincidences, a coincidence time cut to select true coincidence events, a particle identification (PID) cut on the
...
originated from the same reaction vertex at the target (see \DIFaddbegin \DIFadd{online }\DIFaddend Supplemental Material). \\
...
on the cross sections were found (see \DIFaddbegin \DIFadd{online }\DIFaddend Supplemental Material).
...
\DIFdelbegin \DIFdel{$E_{\mathrm{f}}$ is }\DIFdelend \DIFaddbegin \DIFadd{($E_{\mathrm{f}}, p_{\mathrm{f}}$) are }\DIFaddend the final proton energy \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{and momentum, respectively, }\DIFaddend $f_{\mathrm{rec}}\equiv \frac{2p_{\mathrm{f}}^{2}E_{\mathrm{r}}}{2p_{\mathrm{f}}^{2}E_{\mathrm{r}} - (q^{2} - (p_{\mathrm{f}}^{2} + p_{\mathrm{r}}^{2}))E_{\mathrm{f}}}$
...
The \DIFdelbegin \DIFdel{MS calculations used the FF parametrization of Ref. \mbox{%DIFAUXCMD
...
for the proton electric FF parametrization. The JVO calculations used two }\DIFdelend \DIFaddbegin \DIFadd{calculations use }\DIFaddend different FF parametrizations \DIFdelbegin \DIFdel{(GKex05 \mbox{%DIFAUXCMD
...
JVO reduced cross sections.Figure \ref{fig:fig1} shows the results using only the GKex05 parametrization. }\DIFdelend \DIFaddbegin \DIFadd{which can lead to a 6$\%$ change of the theortical cross section.}\DIFaddend \\
%DIF > ----REMOVED ON 10/14/2020----------
%DIF > The MS calculations used the FF parametrization of Ref. \cite{PhysRevC.70.068202} while the JML calculations used the conventional dipole parametrization for
%DIF > the proton and neutron magnetic FF, the Galster \cite{Galster:1971} parametrization
%DIF > for the neutron electric FF, and the results from the Hall A experiment of Ref. \cite{PhysRevLett.88.092301} for the proton electric FF parametrization.
%DIF > The JVO calculations used two different FF parametrizations (GKex05 \cite{PhysRevC.66.045501}, AMT \cite{PhysRevC.76.035205}) where a
%DIF > difference of $\sim5.8-6.6 \%$ was found between the respective
%DIF > JVO reduced cross sections. Figure \ref{fig:fig1} shows the results using only the GKex05 parametrization. \\
