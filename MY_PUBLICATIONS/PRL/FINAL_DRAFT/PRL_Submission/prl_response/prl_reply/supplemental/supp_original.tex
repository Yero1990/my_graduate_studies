...
\noindent Figures \ref{fig:fig1}-\ref{fig:fig5} show the event selection cuts for data (blue hatched) and SIMC (red data points) for the deuteron 80 MeV/c kinematic setting at $\theta_{nq}=35\pm5^{\circ}$.
...
\begin{figure}[!h]
\includegraphics[scale=0.35]{plots/Em_and_Q2_CUT_80MeV_35deg.png}
...
\caption{Same as Fig. \ref{fig:fig4}, but for simulation.}
\label{fig:fig5}
\end{figure}
\clearpage
...
\indent Figure \ref{fig:fig10} shows the missing energy spectrum for the 580 MeV/c setting (left) and the corresponding reconstructed SHMS $z$-vertex (right) for the specified
...
kinematic bin in ($\theta_{nq}$, $p_{\mathrm{r}}$). For illustration purposes, Fig. \ref{fig:fig16} shows the experimental data yield (left) and the spectrometers' phase space volume (right) binned in
