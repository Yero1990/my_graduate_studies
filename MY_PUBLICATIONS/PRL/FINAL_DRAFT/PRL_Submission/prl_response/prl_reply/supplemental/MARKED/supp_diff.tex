...
%DIF DEL supp.tex           Wed Oct 14 19:20:02 2020
%DIF ADD supp_revised.tex   Wed Oct 14 19:20:02 2020
...
energy cut around the deuteron binding energy ($\sim$2.2 MeV) using the missing energy formula\DIFdelbegin \DIFdel{described in the Letter, where }\DIFdelend \DIFaddbegin \DIFadd{, $E_{\mathrm{m}} = \omega - T_{p} - T_{\mathrm{r}}$, where
$\vec{p_{\mathrm{f}}}$ is the final proton momentum, $\vec{q}$ is }\DIFaddend the \DIFdelbegin \DIFdel{recoiling system is assumed to be a }\DIFdelend \DIFaddbegin \DIFadd{3-momentum transfer and $T_{p}$ is the final proton kinetic energy. The recoil particle kinetic energy,
$T_{\mathrm{r}}$, is calculated from the electron and proton 4-momentum vectors assuming an exclusive three-body final state with a recoiling }\DIFaddend neutron.
The peak is not exactly at the deuteron binding energy because energy loss corrections have not been applied to the data nor SIMC. \DIFaddbegin \\
\indent \DIFaddend Figure \ref{fig:Em_Q2_cuts} (right) shows a
...
distribution.\\
\begin{figure}[!h]
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=0.24]{plots/coin_and_eCal_CUT_80MeV_35deg.png}
%DIFDELCMD < %%%
...
\begin{figure}[!h]
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=0.24]{plots/collimator_CUT_80MeV_35deg_data.png}
%DIFDELCMD < %%%
...
\noindent Figures \ref{fig:d2_kin_35deg} and \ref{fig:d2_kin_45deg} show the fundamental kinematic variable distributions for each of the
...
\begin{figure}[!h]
\includegraphics[scale=0.18]{plots/d2_kin_thnq35.png}
...
\end{figure}
\begin{figure}[!h]
\includegraphics[scale=0.18]{plots/d2_kin_thnq45.png}
