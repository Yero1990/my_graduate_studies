...
%DIF DEL supp_revised_v4.tex   Wed Oct 21 14:36:43 2020
%DIF ADD supp_revised_v5.tex   Thu Oct 22 16:42:43 2020
...
\begin{figure}[!h]
\DIFdelbeginFL \DIFdelFL{<<<<<<< HEAD
}%DIFDELCMD < \includegraphics[scale=0.24]{plots/Em_and_Q2_CUT_80MeV_35deg.png}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Event selection cuts on missing energy (left) and 4-momentum transfer (right).}}
%DIFAUXCMD
%DIFDELCMD < \label{fig:Em_Q2_cuts}
%DIFDELCMD < \includegraphics[scale=0.24]{plots/deltaAcc_and_ZtarCUT_80MeV_35deg.png}
%DIFDELCMD < %%%
\DIFdelFL{=======
}\DIFdelendFL \includegraphics[scale=0.33]{plots/Em_and_Q2_CUT_80MeV_35deg.png}
...
\DIFdelbeginFL \DIFdelFL{>>>>>>> 19c4a9ccdd64f67306c82a65bb33aceb646283a5
}\DIFdelendFL \caption{Acceptance cut on HMS momentum fraction (left) and event selection cut on the difference between the $z$-reaction vertex on both spectrometers (right).
...
\begin{figure}[!h]
\DIFdelbeginFL \DIFdelFL{<<<<<<< HEAD
}\DIFdelendFL \includegraphics[scale=0.23]{plots/coin_and_eCal_CUT_80MeV_35deg.png}
...
\begin{figure}[!h]
\DIFaddbeginFL \DIFaddFL{<<<<<<< HEAD
}\DIFaddendFL \includegraphics[scale=0.24]{plots/collimator_CUT_80MeV_35deg_data.png}
\DIFdelbeginFL %DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{(left) Geometrical acceptance cut on reconstructed events projected at the HMS collimator. (right) The SHMS events (correlated with HMS events on left plot),
  projected at the SHMS collimator.}}
%DIFAUXCMD
\DIFdelFL{=======
}%DIFDELCMD < \includegraphics[scale=0.33]{plots/coin_and_eCal_CUT_80MeV_35deg.png}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Event selection cuts on the electron-proton ($ep$) coincidence time (left) and total deposited energy on calorimeter normalized by the particle track momentum (right).}}
%DIFAUXCMD
%DIFDELCMD < \label{fig:coin_ecal_cuts}
%DIFDELCMD < \end{figure}\\
%DIFDELCMD < \indent %%%
\DIFdel{Figure \ref{fig:coin_ecal_cuts} (left) shows a coincidence time cut on the $ep$ coincidence time spectrum formed by requiring a logic signal of
...
%DIFDELCMD < \includegraphics[scale=0.25]{plots/collimator_CUT_80MeV_35deg_data.png}
%DIFDELCMD < %%%
\DIFdelendFL \caption{(left) Geometrical acceptance cut on reconstructed events projected at the HMS collimator. (right) The SHMS events (correlated with HMS events on left plot),
  projected at the SHMS collimator. The $z$-axis (color palette) is in arbitrary units.}
\DIFdelbeginFL \DIFdelFL{>>>>>>> 19c4a9ccdd64f67306c82a65bb33aceb646283a5
}\DIFdelendFL \label{fig:data_coll_cuts}
\end{figure}
\clearpage
...
\begin{figure}[!h]
\DIFdelbeginFL \DIFdelFL{<<<<<<< HEAD
}%DIFDELCMD < \includegraphics[scale=0.24]{plots/collimator_CUT_80MeV_35deg_SIMC.png}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Same as Fig. \ref{fig:data_coll_cuts}, but for simulation.}}
%DIFAUXCMD
\DIFdelFL{=======
}\DIFdelendFL \includegraphics[scale=0.25]{plots/collimator_CUT_80MeV_35deg_SIMC.png}
\caption{Same as Fig. \ref{fig:data_coll_cuts}, but for simulation.  The $z$-axis (color palette) is in arbitrary units.}
\DIFdelbeginFL \DIFdelFL{>>>>>>> 19c4a9ccdd64f67306c82a65bb33aceb646283a5
}\DIFdelendFL \label{fig:simc_coll_cuts}
\end{figure}
...
\caption{(left) Missing energy spectrum for the deuteron 580 MeV/c setting with event selection region corresponding to Aluminum endcaps (in red) and deuteron missing energy peak over
  \DIFdelbeginFL \DIFdelFL{<<<<<<< HEAD
  }\DIFdelendFL a \DIFdelbeginFL \DIFdelFL{40 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{60 }\DIFaddendFL MeV range, each.  \DIFaddbeginFL \DIFaddFL{Inset }\DIFaddendFL (\DIFaddbeginFL \DIFaddFL{left): Missing energy spectrum on a logarithmic scale. (}\DIFaddendFL right) The SHMS $z$-reaction vertex corresponding to
...
\DIFdelbeginFL \DIFdelFL{=======
  ...
  the specified region in the missing energy spectrum.}%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelFL{>>>>>>> 19c4a9ccdd64f67306c82a65bb33aceb646283a5
}\DIFdelendFL \label{fig:tgt_wall}
\end{figure}
...
\DIFdelbegin \DIFdel{<<<<<<< HEAD
}\DIFdelend Laget PWIA and FSI models for systematic effect studies, but the FSI model was ultimately used to correct the data yield. \\
...
%DIFDELCMD < \includegraphics[scale=0.26]{plots/RC_factor.png}
%DIFDELCMD < %%%
\DIFdelFL{>>>>>>> 19c4a9ccdd64f67306c82a65bb33aceb646283a5
}\DIFdelendFL \caption{Radiative correction factor versus neutron recoil momenta, $p_{\mathrm{r}}$, for $\theta_{nq}=35^{\circ}$ (left) and $45^{\circ}$ (right). }
\label{fig:RC_factor}
\end{figure}\\
...
\DIFdelbegin \DIFdel{<<<<<<< HEAD
}\DIFdelend Fig. \ref{fig:BC_factor}. The calculation was done using the Laget PWIA and FSI models for systematic effect studies, but the FSI model was ultimately used to correct the data cross section. \\
\begin{figure}[!ht]
...
%DIFDELCMD < %%%
\DIFdelFL{>>>>>>> 19c4a9ccdd64f67306c82a65bb33aceb646283a5
}\DIFdelendFL \caption{Bin centering correction factor versus neutron recoil momenta, $p_{\mathrm{r}}$, for $\theta_{nq}=35^{\circ}$ (left) and $45^{\circ}$ (right).
...
\end{figure}\\
\DIFdelbegin \DIFdel{<<<<<<< HEAD
}\DIFdelend \indent The uncertainty in the bin-centering correction factor was determined for each  ($\theta_{nq}$, $p_{\mathrm{r}}$) bin
...
}\DIFdelend \section{\large Systematic Uncertainty Studies on Radiative and Bin-Centering Corrections }
...
%\end{figure}
\DIFdelbegin \DIFdel{<<<<<<< HEAD
}\DIFdelend \indent The systematic effect on the cross sections due to model dependency of the radiative and bin-centering corrections was investigated (see Section 5.10 of Ref. \cite{cyero_phdthesis})
\DIFdelbegin \DIFdel{=======
}%DIFDELCMD < \indent %%%
\DIFdel{The systematic effect on the cross sections due to model dependency of the radiative and bin-centering corrections was investigated (see Section 5.10 of Ref. \mbox{%DIFAUXCMD
\cite{cyero_phdthesis}}\hspace{0pt}%DIFAUXCMD
)
>>>>>>> 19c4a9ccdd64f67306c82a65bb33aceb646283a5
}\DIFdelend using the Barlow ratio approach \cite{barlow2002systematic,barlow2017}. In this case, the ratio was calculated from the difference between the experimental cross
...
\DIFdelbegin \DIFdel{<<<<<<< HEAD
}\DIFdelend effect in the measured cross sections and thus no systematic uncertainties due to software cuts were included in the final result. For a detailed discussion of these studies see Section 5.10 of Ref. \cite{cyero_phdthesis}.
\DIFdelbegin \DIFdel{=======
