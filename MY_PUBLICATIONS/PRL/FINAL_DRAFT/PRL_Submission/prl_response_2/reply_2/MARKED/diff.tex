...
%DIF DEL ms.tex           Mon Nov 23 11:37:44 2020
%DIF ADD ms_revised.tex   Tue Nov 24 10:30:49 2020
\pdfoutput=1
...
carried out at Jefferson Lab (JLab) \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{PhysRevLett.113.022501,PhysRevLett.122.172502,PhysRevLett.124.212501} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Subedi1476, PhysRevLett.113.022501,PhysRevLett.122.172502, Schmidt_2020} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend and Brookhaven National Lab (BNL) \cite{PhysRevLett.97.162504}. In these experiments missing momenta up to $\sim1$ GeV/c have been probed and missing momentum distributions
...
\caption{(Color online). The ratio $R(p_{\mathrm{r}}$) is shown in (a)-(c) for $\theta_{nq}=35^{\circ}, 45^{\circ}$ and $75^{\circ}$, respectively, each with a bin width of $\pm 5^{\circ}$.
  ...
  Insets: Blowup of the subfigures \DIFdelbeginFL \DIFdelFL{below $p_{\mathrm{r}}=$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{for $p_{\mathrm{r}}\leq$}\DIFaddendFL 0.7 GeV/c.}
\label{fig:fig2}
\end{figure}
...
experimental data that probe the high momentum region of the $np$ system in the most direct way possible\DIFaddbegin \DIFadd{.
The new data set is also ideal for testing fully relativistic deuteron models based on light-front \mbox{%DIFAUXCMD
\cite{Frankfurt:1981mk} }\hspace{0pt}%DIFAUXCMD
or covariant \mbox{%DIFAUXCMD
\cite{Buck:1979ff} }\hspace{0pt}%DIFAUXCMD
formalisms.
In this respect the current effective field theories (EFT) based models \mbox{%DIFAUXCMD
\cite{Reinert_EFT2018} }\hspace{0pt}%DIFAUXCMD
are non-relativistic and might not have direct relevance to our data}\DIFaddend .
