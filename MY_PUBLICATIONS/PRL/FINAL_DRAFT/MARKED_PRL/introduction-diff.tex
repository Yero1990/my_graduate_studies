...
practically unexplored and not well understood. At such small inter-nucleon distances, the nucleon-nucleon (\DIFdelbegin \DIFdel{NN}\DIFdelend \DIFaddbegin \DIFadd{$NN$}\DIFaddend ) interaction is expected to become repulsive and the interacting
nucleons begin to overlap. The dynamics in this short distance region are directly related to the dynamics of two-nucleon short range correlations (SRC) observed in $A>2$ nuclei \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{PhysRevC.68.014313,PhysRevLett.96.082501,PhysRevLett.99.072501,Fomin_2017,Barack_2019,RevModPhys.89.045002}}\hspace{0pt}%DIFAUXCMD
...
\DIFaddbegin \DIFadd{However, in contrast to SRCs, they were related mostly to the final state structure of nuclei rather than to its initial state (see, e.g., Ref.\mbox{%DIFAUXCMD
\cite{PhysRevLett.84.3045}}\hspace{0pt}%DIFAUXCMD
).}\DIFaddend \\
\indent The most direct way to study the short range structure of the deuteron wave function (or equivalently, its high momentum \DIFdelbegin \DIFdel{components}\DIFdelend \DIFaddbegin \DIFadd{component}\DIFaddend ) is via the exclusive deuteron
electro-disintegration reaction at internal momenta $p_{\mathrm{r}}>300$ MeV/c. For $^{2}\mathrm{H}(e,e'p)n$, within the \DIFdelbegin \DIFdel{PWIA}\DIFdelend \DIFaddbegin \DIFadd{plane wave impulse approximation (PWIA)}\DIFaddend , the virtual photon couples to
...
%DIF > \indent In reality, the ejected nucleon undergoes FSI corresponding to subsequent interactions with the recoiling system. Another possibility is that the
%DIF > photon couples to the virtual meson being exchanged between nucleons (MEC), or that the photon excites a bound nucleon into an isobar resonance state ($\Delta$) which subsequently
%DIF > decays ($\Delta N\rightarrow NN$) via FSI to the ground state (IC).
\indent In \DIFdelbegin \DIFdel{reality}\DIFdelend \DIFaddbegin \DIFadd{addition to PWIA picture}\DIFaddend , the ejected nucleon undergoes \DIFdelbegin \DIFdel{FSI corresponding to subsequent interactions }\DIFdelend \DIFaddbegin \DIFadd{final state interactions (FSI) }\DIFaddend with the recoiling \DIFdelbegin \DIFdel{system. Another possibility is that the photon couples to the virtual meson being exchanged between nucleons }\DIFdelend \DIFaddbegin \DIFadd{nucleon. Other contributing processes are
the photon coupling to the exchanged mesons in the $pn$ system, generating meson exchange currents }\DIFaddend (MEC), or \DIFdelbegin \DIFdel{that the photon excites a }\DIFdelend \DIFaddbegin \DIFadd{the photon exciting the }\DIFaddend bound nucleon into \DIFdelbegin \DIFdel{an isobar resonance state (}\DIFdelend \DIFaddbegin \DIFadd{the
resonating state (mainly }\DIFaddend $\Delta$\DIFdelbegin \DIFdel{) which subsequently
decays ($\Delta N\rightarrow NN$) via FSI to the ground state }\DIFdelend \DIFaddbegin \DIFadd{-isobar) with subsequent $\Delta N \to NN$ rescattering, referred to as isobar currents }\DIFaddend (IC). FSI, MEC and IC can significantly alter the recoiling neutron
...
$\Lambda^{2}\sim 0.8-1 $ (GeV/c)$^{2}$ \cite{Sargsian_2001}. \DIFaddbegin \DIFadd{Note that other meson exchange contributions that take place before the virtual photon interaction are included in the definition of the ground state wave function of the detueron. }\DIFaddend IC can be suppressed kinematically by selecting $x_{\mathrm{Bj}}>1$, where one probes the lower energy ($\omega$) part of the deuteron quasi-elastic peak which is maximally away from the inelastic resonance
electro-production threshold. Previous deuteron electro-disintegration experiments performed at lower $Q^{2}$ ($Q^{2}<1$ (GeV/c)$^{2}$) (\DIFdelbegin \DIFdel{See }\DIFdelend \DIFaddbegin \DIFadd{see }\DIFaddend Section 5 of Ref. \cite{sargsian_2015}) have helped quantify the contributions
from FSI, MEC and IC \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend the $^{2}\mathrm{H}(e,e'p)n$ cross \DIFdelbegin \DIFdel{section }\DIFdelend \DIFaddbegin \DIFadd{sections }\DIFaddend and to determine the kinematics at which they are either suppressed (MEC and IC) or under control (FSI).  \\
...
\indent In the Hall A experiment \cite{PhysRevLett.107.262501}, the pair of high resolution spectrometers (HRS) made it possible to measure the missing momentum dependence of the cross \DIFdelbegin \DIFdel{section }\DIFdelend \DIFaddbegin \DIFadd{sections }\DIFaddend for fixed neutron recoil angles ($\theta_{nq}$)
