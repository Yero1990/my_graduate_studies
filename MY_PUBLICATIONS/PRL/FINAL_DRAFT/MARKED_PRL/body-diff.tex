\indent A 10.6 GeV electron beam was incident on a \DIFdelbegin \DIFdel{10 cm long }\DIFdelend \DIFaddbegin \DIFadd{10-cm-long }\DIFaddend liquid deuterium target (LD$_{2}$). The scattered electron and knocked-out proton were detected in coincidence
...
target boiling inefficiency ($4.2 \%$) \cite{cyero_phdthesis}. The values in parentheses were averaged over all \DIFaddbegin \DIFadd{missing }\DIFaddend momentum settings. \\
...
%The overall systematic uncertainty was determined by the quadrature sum of the normalization and kinematic uncertainties which varied up to 10.6$\%$. This result was then added in quadrature
%to the statistical uncertainty (20-30$\%$ on average) to obtain the final uncertainty on the cross sections. \\
%\onecolumngrid
%\begin{center}
\begin{figure*}[t]
\centering        
...
\indent At $\theta_{nq}=75^{\circ}$ (Fig. \hyperref[fig:fig1]{1(c)}) and $p_{\mathrm{r}}>180$ MeV/c, FSI become the dominant \DIFdelbegin \DIFdel{contributions }\DIFdelend \DIFaddbegin \DIFadd{contribution }\DIFaddend to the cross sections for all models which exhibit a similar
