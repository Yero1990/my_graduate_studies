...
For each spectrometer, a \DIFdelbegin \DIFdel{logical }\DIFdelend \DIFaddbegin \DIFadd{logic }\DIFaddend signal was created from  the coincidence of hits in three of the four scintillator planes. The event trigger was the coincidence of these two signals. \\
...
the \DIFdelbegin \DIFdel{the }\DIFdelend lower and higher missing momentum settings, respectively. At these kinematic settings, the 3-momentum transfer covered a range of $2.4\lesssim|\vec{q}|\lesssim3.2$ GeV/c, which is more than twice the highest neutron recoil momentum
...
energy cut (peak $\sim$ 2.22 MeV for the deuteron) to select true $^{2}\mathrm{H}(e,e'p)n$ coincidences, a coincidence time cut to select true coincidence events, a \DIFdelbegin \DIFdel{PID }\DIFdelend \DIFaddbegin \DIFadd{particle identification (PID) }\DIFaddend cut on the
...
proton loss in HMS ($0.49 \%$), total live time ($3.0 \%$) \DIFdelbegin \DIFdel{, }\DIFdelend and total charge ($2.0\%$) were the same for every setting and thus were added (in quadrature)
...
\indent The systematic uncertainties on the cross \DIFdelbegin \DIFdel{section due to }\DIFdelend \DIFaddbegin \DIFadd{sections due to the }\DIFaddend systematic error on the absolute beam energy and spectrometer angle/momentum settings were
...
The overall systematic uncertainty \DIFdelbegin \DIFdel{in the cross section }\DIFdelend was determined by the
\begin{figure*}[t]
\centering        
...
with a bin width of $\pm 5^{\circ}$. The data are compared to the previous Hall A experiment (red square) results \mbox{%DIFAUXCMD
...
\label{fig:fig1}
\end{figure*}
\noindent quadrature sum of the normalization and kinematic uncertainties \DIFaddbegin \DIFadd{which varied up to 10.6$\%$}\DIFaddend . This result was then added in quadrature
to the statistical uncertainty (20-30$\%$ on average) to obtain the final uncertainty \DIFdelbegin \DIFdel{in the cross section}\DIFdelend \DIFaddbegin \DIFadd{on the cross sections}\DIFaddend . \\
...
over the kinematic bin. The systematic \DIFdelbegin \DIFdel{uncertainty }\DIFdelend \DIFaddbegin \DIFadd{uncertainties }\DIFaddend associated with the radiative and bin-centering corrections \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{were }\DIFaddend investigated using the Laget PWIA and FSI models but negligible effects on the cross sections were found. The experimental and theoretical reduced cross sections were extracted and are defined as follows:
\begin{equation}
\sigma_{\mathrm{red}} \equiv \frac{\sigma_{\mathrm{exp(th)}}}{E_{\mathrm{f}}p_{\mathrm{f}}f_{\mathrm{rec}}\sigma_{\mathrm{cc1}}},
\label{eq:1}
\end{equation}
\noindent where $\sigma_{\mathrm{exp(th)}}$ is the 5-fold experimental (or theoretical) differential cross section \DIFdelbegin \DIFdel{, }\DIFdelend $\frac{d^{5}\sigma}{d\omega d\Omega_{e} d\Omega_{p}}$,
...
how the \DIFdelbegin \DIFdel{NN }\DIFdelend \DIFaddbegin \DIFadd{$NN$ }\DIFaddend potential is modeled based on the empirical \DIFdelbegin \DIFdel{NN }\DIFdelend \DIFaddbegin \DIFadd{$NN$ }\DIFaddend scattering data.
The CD-Bonn model is based on the One-Boson-Exchange Potential (OBEP) approach in which the 
nucleon-meson-meson couplings are constrained to describe the \DIFdelbegin \DIFdel{NN }\DIFdelend \DIFaddbegin \DIFadd{$NN$ }\DIFaddend scattering phase shifts
...
which \DIFdelbegin \DIFdel{require }\DIFdelend \DIFaddbegin \DIFadd{requires }\DIFaddend comparatively few parameters while still producing a high-precision fit to the \DIFdelbegin \DIFdel{NN }\DIFdelend \DIFaddbegin \DIFadd{$NN$ }\DIFaddend scattering data.
...
same \DIFdelbegin \DIFdel{NN }\DIFdelend \DIFaddbegin \DIFadd{$NN$ }\DIFaddend scattering phase-shifts. The major difference between the CD-Bonn and Paris/AV18/WJC2 
...
The \DIFdelbegin \DIFdel{effect }\DIFdelend \DIFaddbegin \DIFadd{effects }\DIFaddend of these local approximations on the \DIFdelbegin \DIFdel{NN }\DIFdelend \DIFaddbegin \DIFadd{$NN$ }\DIFaddend potential are shown in Fig. 2 of Ref. \cite{PhysRevC.63.024001}.\\
...
The agreement at $p_{\mathrm{r}}\leq250$ MeV/c can be understood from the fact that this region corresponds to the long-range part of the \DIFdelbegin \DIFdel{NN }\DIFdelend \DIFaddbegin \DIFadd{$NN$ }\DIFaddend potential where the One Pion Exchange Potential (OPEP)
...
to $p_{\mathrm{r}}\sim700$ MeV/c and $\sim600$ MeV/c, respectively. Furthermore, the agreement between the Halls A and C data \DIFdelbegin \DIFdel{confirm }\DIFdelend \DIFaddbegin \DIFadd{validates }\DIFaddend the Hall A approach of selecting a kinematic
...
missing momenta typically above 800 MeV/c for the MS CD-Bonn calculation while the other models predict still relatively small FSI below 900 MeV/c.
...
At $p_{\mathrm{r}}>300$ MeV/c and $\theta_{nq}=75^{\circ}$, the data were statistically limited as our focus was on the smaller recoil angles. The Hall A data, however, \DIFdelbegin \DIFdel{shows }\DIFdelend \DIFaddbegin \DIFadd{show }\DIFaddend a reasonable agreement with the FSI from all models which gives us confidence in our understanding
...
very high neutron recoil momenta ($p_{\mathrm{r}}>500$ MeV/c) at kinematics where FSI were expected to be small and the cross \DIFdelbegin \DIFdel{section was }\DIFdelend \DIFaddbegin \DIFadd{sections were }\DIFaddend dominated by PWIA and sensitive to the
short range part of the deuteron wave function. The experimental reduced cross sections were extracted and found to be in good agreement with the Hall A data \DIFaddbegin \DIFadd{at lower recoil momenta where they overlap}\DIFaddend .
...
At \DIFaddbegin \DIFadd{the }\DIFaddend higher missing momenta \DIFaddbegin \DIFadd{provided by this experiment }\DIFaddend ($p_{\mathrm{r}}>700$ MeV/c), however, all models were unable to describe the data. 
