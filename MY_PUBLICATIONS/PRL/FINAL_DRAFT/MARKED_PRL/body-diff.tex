...
The overall systematic uncertainty \DIFdelbegin \DIFdel{in the cross section }\DIFdelend was determined by the quadrature sum of the normalization and kinematic uncertainties \DIFaddbegin \DIFadd{which varied up to 10.6$\%$}\DIFaddend . This result was then added in quadrature
...
over the kinematic bin. The
\DIFdelbegin \DIFdel{systematic uncertainty }\DIFdelend %DIF > \onecolumngrid
%DIF > \begin{center}

%DIF > \end{center}
%DIF > \twocolumngrid
\noindent \DIFadd{systematic uncertainties }\DIFaddend associated with the radiative and bin-centering corrections \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{were }\DIFaddend investigated using the Laget PWIA and FSI models but negligible effects on the cross sections were found. The experimental and theoretical reduced cross sections were extracted and are defined as follows:
\begin{equation}
...
which \DIFdelbegin \DIFdel{require }\DIFdelend \DIFaddbegin \DIFadd{requires }\DIFaddend comparatively few parameters while still producing a high-precision fit to the NN scattering data.
...
The \DIFdelbegin \DIFdel{effect }\DIFdelend \DIFaddbegin \DIFadd{effects }\DIFaddend of these local approximations on the NN potential are shown in Fig. 2 of Ref. \cite{PhysRevC.63.024001}.\\
