...
to define the overall normalization uncertainty ($\leq$ 8.4$\%$).\\
...
The total uncertainty was defined as the quadrature sum of the normalization ($\leq$ 8.4$\%$), kinematic ($\leq$ 6.5$\%$) and statistical ($\sim20-30\%$ on average) uncertainties
to obtain the final uncertainty on the cross sections.\\
